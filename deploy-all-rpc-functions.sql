...
-- ============================================
-- Este script cria TODAS as funções RPC necessárias com SECURITY DEFINER
-- Execute APÓS confirmar que RLS está desabilitado
...
BEGIN
    ...
    ELSE
        RAISE NOTICE '✅ OK: RLS desabilitado, prosseguindo com criação das funções...';
    END IF;
END $$;
...
-- Listar Eventos
CREATE OR REPLACE FUNCTION public.scribia_get_eventos(
  p_usuario_id UUID
)
RETURNS JSON
LANGUAGE plpgsql
...
-- Atualizar Livebook
CREATE OR REPLACE FUNCTION public.scribia_update_livebook(
  p_livebook_id UUID,
  p_usuario_id UUID,
  p_status TEXT DEFAULT NULL,
